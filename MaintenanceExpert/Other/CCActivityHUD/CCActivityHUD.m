...
- (void)dismissWithNoView:(NSString *)text delay:(CGFloat)delay {
    if (self.subviews) {
        ...
        [self addDisappearAnimationWithDelay:0];
    }
}
    ...
    if (self.superview) {
        ...
        if (text != nil || text.length != 0) {
            [UIView animateWithDuration:0.5 animations:^{
                self.frame = CGRectMake(0, 0, TEXT_WIDTH, 8+[self heightForText:text]+4);
                self.center = BoundsCenterFor(Screen);
                
                UILabel *textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 8, ViewFrameWidth, [self heightForText:text])];
                textLabel.numberOfLines = 0;
                textLabel.font = [UIFont systemFontOfSize:TEXT_FONT_SIZE];
                textLabel.textColor = [self inverseColorFor:self.backgroundColor];
                textLabel.textAlignment = NSTextAlignmentCenter;
...
#warning HUD.frame (originalFrame) 这里修改
