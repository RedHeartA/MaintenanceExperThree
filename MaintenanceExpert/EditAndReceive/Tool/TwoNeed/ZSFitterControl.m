...
#define TITLE_COLOR ColorWithRGBA(80, 150, 260, 1)
...
- (id) initWithFrame:(CGRect) frame Titles:(NSArray *) titles {
    if (self = [super initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, 70)]) {
        ...
        UIImage *image1 = [UIImage imageNamed:@"slider_btn_nomal"];
        UIImage *image2 = [UIImage imageNamed:@"slider_btn_nomal"];
...
- (void) animateTitlesToIndex:(int) index {
    ...
    for (i = 0; i < titlesArr.count; i++) {
        ...
        if (i == index) {
            ...
            [lbl setTextColor: ColorWithRGBA(59, 181, 245, 1)];
            [lbl setAlpha:1];
        }else{
            //未选中时label颜色
