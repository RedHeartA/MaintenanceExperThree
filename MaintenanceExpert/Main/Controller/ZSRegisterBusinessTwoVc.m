...
@interface ZSRegisterBusinessTwoVc ()<UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, STPickerAreaDelegate>
...
@implementation ZSRegisterBusinessTwoVc
...
- (UIButton *)creatBussinessButton {
    ...
    [bussinessButton setTitle:@"提 交" forState:UIControlStateNormal];
    [bussinessButton addTarget:self action:@selector(bussinessTwoButtonClick) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:bussinessButton];
    
    return bussinessButton;
}

- (void)bussinessTwoButtonClick {
...
- (UITableView *)creatTableView {
    ...
    self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, CELL_COUNT * CELL_HEIGHT + 30) style:UITableViewStylePlain];
